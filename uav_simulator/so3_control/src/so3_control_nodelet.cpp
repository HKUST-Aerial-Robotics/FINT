...
public:
  SO3ControlNodelet()
    ...
    last_cmd_time_(0.0),
    last_odom_time_(0.0),
    cur_cmd_state_(cmd_states::INIT)
  {
  }
...
private:
  ...
  ros::Time last_cmd_time_, last_odom_time_;
};
...
void SO3ControlNodelet::position_cmd_callback(const quadrotor_msgs::PositionCommand::ConstPtr& cmd) {
  ...
  last_cmd_time_ = last_odom_time_;

  cur_cmd_state_ = cmd_states::TRAJ;
...
void SO3ControlNodelet::odom_callback(const nav_msgs::Odometry::ConstPtr& odom) {
  ...
  last_odom_time_ = odom->header.stamp;
  ...
  switch(cur_cmd_state_){
    case cmd_states::INIT:{
      hover_pos = position;
      des_pos_ = position;
      des_vel_.setZero();
      des_acc_.setZero();
      cur_cmd_state_ = cmd_states::HOVER;
      publishSO3Command();
      break;
    }
    
    case cmd_states::HOVER:{
      des_pos_ = hover_pos;
      des_vel_.setZero();
      des_acc_.setZero();
      publishSO3Command();
    ...
    case cmd_states::TRAJ:{
      if((odom->header.stamp - last_cmd_time_).toSec() > 0.2){
        hover_pos = position;
        des_pos_ = position;
        des_vel_.setZero();
        des_acc_.setZero();
  ...
  //   ROS_ERROR_STREAM("hover position: "<<hover_pos.transpose());
...
void SO3ControlNodelet::onInit(void) {
  ...
  n.param("gains/pos/x", kx_[0], 2.0);
  n.param("gains/pos/y", kx_[1], 2.0);
  n.param("gains/pos/z", kx_[2], 2.0);

  n.param("gains/vel/x", kv_[0], 2.0);
  n.param("gains/vel/y", kv_[1], 2.0);
  n.param("gains/vel/z", kv_[2], 2.0);
