...
public:
    ...
    Eigen::VectorXd& getKnotRef();
    Eigen::VectorXd& getKnotLength();
    void computeKnotLength(const double &res = 5e-3);
    Eigen::MatrixXd getControlPoint();
    double getKnotSpan();
    void getTimeSpan(double &um, double &um_p);
    double getTimeFromLength(const double& length, const double& res = 5e-3);
    ...
    static void parameterizeToBspline(const double &ts, const vector<double> &point_set,
                                              const vector<double> &start_end_derivative,
                                              const int &degree, Eigen::VectorXd &ctrl_pts);

    static void solveFirst3CtrlPts(const double &ts, const vector<Eigen::Vector3d> &start, Eigen::Matrix3d &ctrl_pts);
    static void solveFirst3CtrlPts(const double &ts, const vector<double> &start, Eigen::Vector3d &ctrl_pts);
    ...
    // Evaluation
    double getTimeSum();
    double getLength(const double &dur, const double &res = 0.01);
...
private:
    ...
    Eigen::VectorXd knot_length_; // knots curve length
