...
double odom_tf_q_w, odom_tf_q_x, odom_tf_q_y, odom_tf_q_z;
double odom_tf_t_x, odom_tf_t_y, odom_tf_t_z;
...
void odom_callback(const nav_msgs::Odometry::ConstPtr& msg) {
  ...
  pose.rows(0,2) = quaternion_to_R(odom_tf_q) * pose.rows(0,2) + odom_tf_t;
  q = R_to_quaternion(quaternion_to_R(odom_tf_q) * quaternion_to_R(q));
  pose.rows(3, 5) = R_to_ypr(quaternion_to_R(q));
...
void pose_callback(const geometry_msgs::PoseStamped::ConstPtr& msg) {
  ...
  odom_ptr->header = msg->header;
  odom_ptr->pose.pose = msg->pose;

  odom_callback(odom_ptr);
}
...
int main(int argc, char** argv) {
  ...
  n.param("odom_tf_q_w", odom_tf_q_w, 1.0);
  n.param("odom_tf_q_x", odom_tf_q_x, 0.0);
  n.param("odom_tf_q_y", odom_tf_q_y, 0.0);
  n.param("odom_tf_q_z", odom_tf_q_z, 0.0);

  n.param("odom_tf_t_x", odom_tf_t_x, 0.0);
  n.param("odom_tf_t_y", odom_tf_t_y, 0.0);
  n.param("odom_tf_t_z", odom_tf_t_z, 0.0);

  odom_tf_q(0) = odom_tf_q_w;
  odom_tf_q(1) = odom_tf_q_x;
  odom_tf_q(2) = odom_tf_q_y;
  odom_tf_q(3) = odom_tf_q_z;

  odom_tf_t(0) = odom_tf_t_x;
  odom_tf_t(1) = odom_tf_t_y;
  odom_tf_t(2) = odom_tf_t_z;


  ros::Subscriber sub_odom = n.subscribe("odom", 100, odom_callback);

  ros::Subscriber sub_pose = n.subscribe("sub_pose", 100, pose_callback);
